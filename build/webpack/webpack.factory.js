...
const configurationFactory = () => {
  ...
  const rteStyles = browserConfig(baseOutput({
    entry: path.resolve(dirs.source, 'rte.jsx'),
    outputStyle: '/assets/rte.css'
  }));

  const rtePage = browserConfig(baseOutput({
    entry: path.resolve(dirs.source, 'rte-page.jsx'),
    outputStyle: '/assets/rte-page.css'
  }));

  const devScripts = browserConfig(baseOutput({
    entry: path.resolve(dirs.source, 'devScripts.js'),
    outputScript: '/assets/devScripts.js',
  }));

  const browserScripts = browserConfig(baseOutput({
    entry: ['whatwg-fetch', path.resolve(dirs.source, 'scripts.js')],
    outputScript: '/assets/scripts.js',
  }));

  const browserHeadScripts = browserConfig(baseOutput({
    entry: path.resolve(dirs.source, 'head.js'),
